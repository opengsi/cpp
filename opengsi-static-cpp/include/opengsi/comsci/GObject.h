...
public:
    ...
    operator>>(BaseIStream &theStream, this_reference theSelf) {
        return theSelf._readStream(theStream);
    }

protected:

    void _printMessage(const BasicString &theErrorCode, const BasicString &theFunctionName) const {
...
private:
    ...
    BasicString __getMessage(const BasicString& theErrorCode, const BasicString& theFunctionName, const BasicString& theNameSpace) const {
